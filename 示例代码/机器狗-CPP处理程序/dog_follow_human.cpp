...
public:
    ...
    bool computeControl(const UWB2DData& uwb_data, 
        ...
        // 计算人的速度大小
        double human_speed_magnitude = std::sqrt(human_vx * human_vx + human_vy * human_vy);
        
        // 计算人的径向速度（沿着机器狗到人的方向）
        // 如果EKF估计的速度接近0（人可能静止或数据不稳定），使用标准速度
        double human_radial_speed = 0.0;
        constexpr double MIN_SPEED_THRESHOLD = 5.0;  // 最小速度阈值 (cm/s)
        
        if (human_speed_magnitude > MIN_SPEED_THRESHOLD) {
            // 使用EKF估计的速度
            human_radial_speed = human_speed_magnitude;
        } else {
            // 人可能静止，使用标准速度
        ...
        // 计算人的角向速度（需要从xy速度分解）
        // 角速度 = 垂直于径向的速度分量 / 距离
        // 假设x是左右方向（垂直于径向），y是前后方向（径向）
        double human_angular_speed = 0.0;
        if (now_distance > 10.0) {  // 避免除以零
            if (human_speed_magnitude > MIN_SPEED_THRESHOLD) {
                // 使用EKF估计的角速度：角速度 = vx / distance (弧度/秒 -> 度/秒)
        ...
        if (distance_diff > 0) {
            ...
            radial_at_max_speed_ = false;  // 重置最大速度状态
        ...
        if (angle_diff > FollowConfig::MIN_ANGLE) {
            ...
            if (angular_speed_cmd > FollowConfig::MAX_ANGULAR_SPEED) {
                ...
                // 之前在最大角速度，检查是否应该减速
                // 当角度差小于 1.1 * MIN_ANGLE 时开始减速
                double decel_angle_threshold = FollowConfig::MIN_ANGLE * FollowConfig::ANGLE_DECEL_FACTOR;
                if (angle_diff < decel_angle_threshold) {
                    angular_at_max_speed_ = false;
                } else {
                    // 保持最大角速度
                    angular_speed_cmd = FollowConfig::MAX_ANGULAR_SPEED;
